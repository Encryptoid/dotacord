...
pub async fn update_server_sub_week(server_id: i64, is_sub_week: bool) -> Result<(), Error> {
    let txn = database_access::get_transaction().await?;
    let server = Server::find_by_id(server_id).one(&txn).await?;
...
pub async fn update_server_sub_month(server_id: i64, is_sub_month: bool) -> Result<(), Error> {
    let txn = database_access::get_transaction().await?;
    let server = Server::find_by_id(server_id).one(&txn).await?;
